...
class total_dataset(data.Dataset):
    def __init__(self,root='./dataset',train=True,neg=False,norm=True,exp_case=[1,2,3]):
        mix = MixQuality(root=root+'/mixquality/',train=train,neg=neg,norm=norm,exp_case=exp_case)
        self.x = mix.x
        self.y = mix.y
