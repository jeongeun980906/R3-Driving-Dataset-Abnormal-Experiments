...
DIR = './res/VAE/{}/'.format(args.id)
DIR2 = './res/VAE/{}/ckpt/'.format(args.id)
try:
    os.mkdir('./res/VAE')
except:
    pass
...
train_l2, test_l2 = Solver.train_VAE(f)
log.train_res(train_l2,test_l2)

id_eval = Solver.eval_ood_VAE(Solver.test_e_iter,'cuda')
ood_eval = Solver.eval_ood_VAE(Solver.test_n_iter,'cuda')
