...
parser.add_argument('--plot_hist', type=int,default=0,help='plot histogram')
...
for m in method:
    ...
    query_ratio[m] = np.asarray(data['query_ratio']).T
    temp1 = data['id_eval']
    temp2 = data['ood_eval']
...
plt.title("Query Ratio")
EXP_CASE_NAME = ['FMTC','Highway','Road']
for i,m in enumerate(method):
    for k in range(args.query_step):
        bottom = 0
        for j in range(3):
            if k==0 and i==0:
                plt.bar(k + 0.2*(i-1.5),query_ratio[m][j][k],width=0.2,bottom=bottom,color=color[i][j],label = EXP_CASE_NAME[j])
            else:      
                plt.bar(k + 0.2*(i-1.5),query_ratio[m][j][k],width=0.2,bottom=bottom,color=color[i][j])
            bottom += query_ratio[m][j][k]
plt.xlabel("Query Step")
plt.ylabel("L2")
leg = plt.legend(bbox_to_anchor=(1.05, 1),loc=2, borderaxespad=0.)
leg.set_title('Expert Data Case')
plt.xticks(x_range)
...
if args.plot_hist:
    ...
    for i,m in enumerate(method):
        for k,j in enumerate(['epis_','alea_','pi_entropy_']):
            ...
            case1 = np.where(case==4)[0]
            case1 = ood_ar[case1]
            case2 = np.where(case==5)[0]
            case2 = ood_ar[case2]
            case3 = np.where(case==6)[0]
            case3 = ood_ar[case3]
